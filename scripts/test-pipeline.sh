...
# NOTE: only checks service reachability + the AI engine functional test.
# The full scenario matrix is covered by scripts/smoke_test.py and the pytest suite.
set -euo pipefail
...
check_http() {
    ...
    # Accept the expected code, or any 2xx/3xx (redirects/self-signed setups).
    case "$code" in
        2??|3??) pass "$name ($url) → HTTP $code" ;;
        *)
            if [ "$code" = "$expected_code" ]; then
                pass "$name ($url) → HTTP $code (expected)"
            else
...
if curl -sf "http://localhost:8888/health" >/dev/null 2>&1; then
    ...
    MODEL=$(echo "$AI_RESPONSE" | python3 -c "import sys,json; print(json.load(sys.stdin).get('model','unknown'))" 2>/dev/null || echo "unknown")
