#!/usr/bin/env python3
"""
Send synthetic test alerts to the AI SOC engine to verify the pipeline.

Usage: python send-test-alert.py [scenario]
Scenarios: ssh-bruteforce, port-scan, web-attack, malware, data-exfil, false-positive
...
TEST_ALERTS = {
    ...
    "malware": {
        ...
        "raw_log": (
            ...
            "SHA256: 3f4a8b2c1d9e0000000000000000000000000000000000000000000000000000 | "
            "VirusTotal: 45/72 engines | Process spawned: cmd.exe"
        ),
        "misp_context": {
            "found": True,
    ...
    "data-exfil": {
        ...
        "misp_context": {"found": True, "tags": ["c2", "dns-tunneling"], "threat_level": "high"},
    },
    "false-positive": {
        "alert_id": "TEST-006",
        "source": "wazuh",
...
def send_alert(scenario: str) -> None:
    ...
    for e in result["evidence"]:
        print(f"  - {e}")
    print(f"\nProcessing time: {result['processing_time_ms']}ms | Model: {result['model']}")
