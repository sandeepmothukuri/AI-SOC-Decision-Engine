...
> ⚠️ The JSON files in `shuffle-workflows/` are **reference specifications**,
> not Shuffle-native exports — Shuffle cannot import them as-is. Rebuild the
> same logic in the Shuffle UI (see `shuffle-workflows/README.md`).
...
1. Log into TheHive → `http://localhost:9000`
   ...
   - Password: `secret` (TheHive 5 seeds the first admin user on first login —
     **verify the exact credentials for your image version**)
...
> The AI engine posts to TheHive's **`/api/v1/case`** endpoint (TheHive 5).
> The engine's model, thresholds, approval mode and timeouts are configured in
> `ai-engine/config.yaml` (env-overridable, see `ai-engine/.env.example`).
...
# TheHive 5 path (verify against your version; the old /api/case/template is TH4)
curl -X POST http://localhost:9000/api/v1/case/template \
  -H "Authorization: Bearer $THEHIVE_API_KEY" \
  -H "Content-Type: application/json" \
  -d @thehive-config/case-templates.json
```

> The `thehive-config/case-templates.json` shape has **not been verified**
> against a live TheHive 5 CaseTemplate schema — it may need `displayName`,
> task `order`, and page fields added.
...
# Run the full test-suite (unit + integration; requires pytest)
python3 -m pytest tests/ -q

# End-to-end smoke test with the full scenario matrix + metrics report
python3 scripts/smoke_test.py

# Send a single synthetic alert to the engine
